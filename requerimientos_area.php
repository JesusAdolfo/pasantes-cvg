...
<span>La formula de "Requerimientos X Áreas" es la siguiente:</span>
<span>	
	<table class="fraction" align="center" cellpadding="0" cellspacing="0">
		<tr>
			<td rowspan="2" nowrap="nowrap">
				<i>Requerimientos X Áreas</i> =  
			</td><td nowrap="nowrap">
				<i>Nro de Pasantes X Área <i>
			</td>
		</tr><tr>
			<td class="upper_line">
...
function procesar(){
	...
	if(isset($_POST['submit'])){
		...
		$query = pg_query("SELECT cod_area
						   FROM areas");
		...
		if($num == 0){
			...
			while ($i < $num) {
				$cod_area = pg_fetch_result($query, $i, "cod_area");
				
				$result2 = pg_query("SELECT COUNT(area_desempeno) 
									FROM pasantias
									WHERE (extract (year FROM fecha_inicio)=$anio OR extract (year FROM fecha_fin) = $anio)
									AND especialidad=$cod_area");
				...
				<span style="margin-bottom:0px;">
					<i>(<u><?php get_nombre_area($cod_area);?></u>)</i> 
				</span>
				<span>
					<table class="fraction" align="center" cellpadding="0" cellspacing="0" >
					<tr>
...
<script type="text/javascript">
	 $(document).ready(function() {
      $('#titulo1').text("Sistema de Control de Pasantías > Indicadores > Requerimientos X Área").html;
    });
</script>
<script>
